...
## Paper Benchmark Build

The paper version of FutureCast contains **484,435 instances from 26 real-world sources across eight domain families**. It represents each forecasting case as a unified JSONL record with a forecast origin, target history, sampling information, visible context, and hidden future target. Context is organized into observed covariates, known-future covariates, static attributes, textual descriptions, and event information under a leakage-safe forecast-time boundary.

The current repository is being updated toward this full paper release. The lightweight file-processing preview documented below covers the subset currently packaged with the public toolkit. See the [FutureCast project website](https://ustc-time-series.github.io/future-cast/) for the benchmark overview.
